...
class Projectile {
   ...
   */
  move(){
    if(_coordinate['row'] >= gamesize-1 || _coordinate['row'] <= 0 ) _model.removeProjectile(this);
    else {  _coordinate['row'] += _direction;
  ...
  /**
   * Überprüft dieses [Projectile] drauf, ob es sich auf dem gleichen Feld wie ein [EnemyShip] befindet.
  ...
  detectCollision(){
    ...
    for(EnemyShip enemy in enemies){
      for(Map<String, int> part in enemy.parts){
        if((part['row'] == this._coordinate['row']) && (part['col'] == this._coordinate['col'])){
